...
node("${env.SELF_SERVICE_NODE}") {
	try {
		...
		stage('Invoke playbook') { 
			...
			dir ("${playbooksDir}/bastion-setup") {
			    withCredentials([usernamePassword(credentialsId: svnCredentialID, passwordVariable: 'svnPassword', usernameVariable: 'svnUsername')]) {
				 	sh "ansible-playbook -i ${hostsFile} setup-bastion-host.yml \
				 	-e svn_username=${svnUsername} -e svn_password=${svnPassword} "
			    }
			}
		}
	} finally {
